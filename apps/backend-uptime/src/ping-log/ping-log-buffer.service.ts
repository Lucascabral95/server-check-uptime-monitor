...
export class PingLogBufferService implements OnModuleInit, OnModuleDestroy {
  ...
  add(log: PingLogBuffer): boolean {
    ...
    if (totalBufferSize >= CONFIG.MAX_BUFFER_SIZE) {
      ...
      this.logger.warn(`Buffer full (${totalBufferSize}/${CONFIG.MAX_BUFFER_SIZE}), dropping log`);
      return false;
    }
  ...
  async flush(): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(`Flush completed in ${this.stats.lastFlushDuration}ms`);
    } catch (error) {
  ...
  private async flushRetryBuffer(): Promise<void> {
    ...
    this.logger.debug(`Retrying ${logsToRetry.length} logs from retry buffer`);
  ...
  async flushFinal(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Final flush failed, ${totalLogs} logs may be lost: ${error.message}`);
  ...
  async healthCheck(): Promise<boolean> {
    ...
    // A normal flush temporarily holds the lock; that is healthy work,
    // not a readiness failure. Only sustained backpressure is unhealthy.
    const isHealthy = totalSize < CONFIG.MAX_BUFFER_SIZE * 0.8;

    if (!isHealthy) {
      this.logger.warn(`Health check failed: buffer=${totalSize}, locked=${this.isFlushingLock}`);
    }
